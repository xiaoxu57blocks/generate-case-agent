...
description: Test completion summary and knowledge extraction phase. Invoke after test-runner reports all tests pass. Reads all artifact files, audits existing knowledge for overlap and staleness, extracts only reusable abstract patterns that caused slowness or inaccuracy, and updates AGENTS.md / Codex context files when appropriate.
...
the YAML `tools` and `model` metadata above. In Codex, prefer project knowledge
locations that Codex will read: `AGENTS.md` and `.codex/context/*.md`. Use
...
```bash
# Search Codex/project instruction files for related content
...
grep -rn "{keyword}" .codex/context/ .claude/agents/ 2>/dev/null || true

# Search project facts if present
grep -rn "{keyword}" .codex/context/project-facts.md .claude/context/project-facts.md 2>/dev/null || true
```
...
Is it specific to this project's page structure, roles, or connectors?
  → YES: Add to AGENTS.md or `.codex/context/coding-rules.md` under the relevant section

Did a phase make a systematic mistake that led to the loop?
  → YES: Edit the corresponding project-local Codex reference or workflow doc — add the corrective rule

Is it a user preference or collaboration pattern (not a technical rule)?
  → YES: Save only if the current Codex environment has an explicit memory location; otherwise include it in the session summary

Is it a project-level fact (feature flag, data dependency, env constraint)?
  → YES: Write to `.codex/context/project-facts.md` (create if absent); mirror to `.claude/context/project-facts.md` only if the project uses Claude Code too
...
| Detailed coding rules (target project) | `.codex/context/coding-rules.md` preferred; `.claude/context/coding-rules.md` compatibility fallback |
| Project-level facts (feature flags, data deps, env constraints) | `.codex/context/project-facts.md` preferred; `.claude/context/project-facts.md` compatibility fallback |
| Analyst phase rules | project-local Codex workflow docs, if present |
| Architect phase rules | project-local Codex workflow docs, if present |
| Coder phase rules | project-local Codex workflow docs, if present |
| Runner phase rules | project-local Codex workflow docs, if present |
| Personal collaboration prefs | only an explicit Codex memory location provided by the user |
...
#### Saved to .codex/context/project-facts.md
- {fact}: {what was saved}
...
- **Prefer AGENTS.md / `.codex/context` over memory** for technical patterns — project files are available to future Codex runs, while memory requires explicit support and recall.
