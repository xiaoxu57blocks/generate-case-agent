...
if [ "$OPENCODE_MODE" -eq 1 ] && [ -f "AGENTS.md" ]; then
  ...
  if [ "$OPENCODE_MODE" -eq 1 ]; then
    note_warn "project_instructions" "No AGENTS.md or CLAUDE.md at project root. Recommended: write AGENTS.md describing project structure and conventions — OpenCode reads it before designing a test."
  else
...
# ── 7. coding-rules.md ───────────────────────────────────────────────────────
if [ "$OPENCODE_MODE" -eq 1 ] && [ -f ".opencode/context/coding-rules.md" ]; then
  RULES_FILE=".opencode/context/coding-rules.md"
  ...
  if [ "$OPENCODE_MODE" -eq 1 ]; then
    note_warn "coding_rules" "No .opencode/context/coding-rules.md found. Architect will fall back to AGENTS.md/CLAUDE.md only. Recommended: copy the harness template and adapt to your UI library."
  else
...
MCP_FOUND=""
for cand in .mcp.json "$HOME/.opencode/mcp.json" "$HOME/.config/opencode/mcp.json" "$HOME/.claude/mcp.json" "$HOME/.config/claude/mcp.json"; do
  if [ -f "$cand" ]; then
...
if [ -z "$MISSING_SCRIPTS" ]; then
  ...
  if [ "$OPENCODE_MODE" -eq 1 ]; then
    note_error "harness_scripts" "Missing or non-executable harness scripts:$MISSING_SCRIPTS. Copy the bundled scripts from the playwright-test-harness OpenCode skill into this project's scripts/ directory, then run: chmod +x scripts/*.sh"
  else
...
# ── 10. Harness agents / OpenCode skill mode ────────────────────────────────────
if [ "$OPENCODE_MODE" -eq 1 ]; then
  note_info "opencode_skill" "OpenCode mode enabled. .claude/agents are not required; use the playwright-test-harness OpenCode skill references."
else
  MISSING_AGENTS=""
