...
description: Use this OpenCode skill when the user wants to add, update, or generate Playwright E2E tests from a screenshot, spreadsheet row, CSV test case, or natural-language test case. It adapts the Claude Code harness workflow to OpenCode with staged artifacts: analyst, architect, coder, runner, and summarizer.
---

# Playwright Test Harness for OpenCode

Use this skill inside a real Playwright target project. Do not generate tests in
the harness repository itself.

This skill adapts the Claude Code Playwright Test Harness to OpenCode. Preserve
the important behavior: each phase has a narrow role, writes an artifact to
disk, and the next phase reads only that artifact plus the target project files.
...
0. Bootstrap helper scripts from this skill into the target project if they are
   ...
   `.opencode/context/coding-rules.md` from `references/coding-rules.md` and
   adapt it to the target app.
...
- Store OpenCode project facts in `.opencode/context/project-facts.md` when
  possible. Use `.codex/context/` or `.claude/context/` only for projects that
  intentionally share knowledge across tools.
...
- Keep the harness project-agnostic. Target-project business rules belong in the
  target project's `AGENTS.md`, `.opencode/context/`, or test docs.

## DeepSeek Model Use

OpenCode selects the model outside the skill with `--model provider/model`.
Use the DeepSeek V4 Pro provider/model string configured in your OpenCode
environment, for example:

```bash
opencode run --model deepseek/deepseek-v4-pro --agent build "使用 playwright-test-harness，新增 TC-050 Playwright 测试"
```

If your DeepSeek access is exposed through an OpenAI-compatible provider, the
model name may differ, such as `openai/deepseek-v4-pro`. Confirm with
